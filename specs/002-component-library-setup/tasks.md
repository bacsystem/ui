...
- [x] T059 [US1] Add `outline` prop to `Badge`: transparent background + colored border; filled variants updated with subtle border for white-background contrast (FR-032): `--color-primary-50` background + `--color-primary-400` left-border accent on first cell; CSS transition ≤ 150 ms; hover disabled during loading/empty states (FR-031)
