...
- [x] T019 [P] [US1] Create `Avatar` component in `packages/ui/src/components/Avatar/Avatar.tsx`: 5 sizes; fallback chain src→initials→`User` icon; export `AvatarProps`
...
- [x] T032 [US3] Create `useBreakpoint` hook in `packages/ui/src/hooks/useBreakpoint.ts`: returns `{ isMobile, isTablet, isDesktop, current }`; thresholds: sm (0–767px), md (768–1023px), lg (1024–1279px), xl (1280px+); uses `ResizeObserver` or `window.matchMedia`; automatic cleanup on unmount; SSR-safe
...
- [x] T034 [US4] Create `.github/workflows/publish.yml`: trigger `push.tags: ['v*']`; `permissions: packages: write`; steps: checkout → setup-node (node 20, registry `npm.pkg.github.com`, scope `@bacsystem`) → `npm ci` → `npm run build:ui` → `npm publish --workspace=packages/ui`; env `NODE_AUTH_TOKEN: ${{ secrets.GITHUB_TOKEN }}`
...
- [x] T051 [P] Update `README.md` with "Demo" section: `npm install` + `npm run dev` → `localhost:3000`; add "Install" section pointing to GitHub Packages + `.npmrc` setup
...
- [x] T058 [US1] Add `DataTable` row hover state: `--color-primary-100` background + `--color-primary-600` left-border accent on first cell; CSS transition ≤ 150 ms; hover disabled during loading/empty states (FR-031)
- [x] T059 [US1] Add `outline` prop to `Badge`: transparent background + colored border; filled variants updated with subtle border for white-background contrast (FR-032)
- [x] T060 [P] [US1] Add `appearance` prop (`'filled' | 'outline' | 'soft' | 'link'`) to `Button` in `packages/ui/src/components/Button/Button.tsx`; export `ButtonAppearance` type; keep `outline` boolean for backward compat; add CSS for all 4 appearances × 6 variants in `apps/demo/app/globals.css` (FR-032)
- [x] T061 [P] [US1] Add `appearance` prop (`'soft' | 'filled' | 'outline'`) to `Badge`, `Alert`, `Avatar`, `StatCard`; export `*Appearance` types; add CSS for filled/outline rows × all variants in `apps/demo/app/globals.css` (FR-033)
